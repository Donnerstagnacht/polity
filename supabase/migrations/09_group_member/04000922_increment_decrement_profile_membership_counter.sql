DROP FUNCTION IF EXISTS authenticated_access.increment_profile_group_membership_counter(
    user_id uuid
);
CREATE OR REPLACE FUNCTION authenticated_access.increment_profile_group_membership_counter(
    user_id uuid
)
    RETURNS void
    LANGUAGE plpgsql
