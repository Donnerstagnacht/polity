DROP FUNCTION IF EXISTS authenticated_access.increment_group_follower_counter(
    group_id_in uuid
);
CREATE OR REPLACE FUNCTION authenticated_access.increment_group_follower_counter(
    group_id_in uuid
)
    RETURNS void
    LANGUAGE plpgsql
