...
export class FollowersOfUserActionService {
    ...
    public async selectFollowersOfUser(): Promise<any> {
        await this.followersOfUserStoreService.followersOfUser.wrapSelectFunction(async (): Promise<void> => {
            const followerResponse: PostgrestSingleResponse<SupabaseArrayReturn<'select_follower_of_user'>> = await this.supabaseClient.rpc(
                'select_follower_of_user'
            )
            .throwOnError()
            if (followerResponse.data) {
                const finalArray: SupabaseArrayReturn<'select_follower_of_user'> = await this.profileActionService.transformImageNamesToUrls(followerResponse.data, 'profile_image')
    ...
    public async removeFollowerOfUser(userId: string): Promise<any> {
        await this.followersOfUserStoreService.followersOfUser.wrapUpdateFunction(async (): Promise<void> => {
            const response: PostgrestSingleResponse<SupabaseArrayReturn<'remove_follower_transaction'>> = await this.supabaseClient.rpc(
